...
function enterpriseDashboard(){const u=Linggong.ui;return `<p class="crumb">首页 / 企业工作台</p>${u.header("企业合作工作台","从达人筛选、合作任务发布到结算支付，统一管理每一次品牌合作。",`<button class="primary" onclick="Linggong.go('publish')">发布合作需求</button>`)}<div class="metrics">${[["达人库总数","30 万+","workers"],["进行中合作","36","tasks"],["待结算金额","¥128,600","orders"]].map(x=>`<button class="metric" onclick="Linggong.go('${x[2]}')"><span>${x[0]}</span><strong>${x[1]}</strong><small>点击查看详情</small></button>`).join("")}</div><div class="actions">${[["创建营销项目","projects"],["发布合作需求","publish"],["创建结算订单","orders"]].map(x=>`<button class="action" onclick="Linggong.go('${x[1]}')"><small>快捷操作</small><b>${x[0]}</b><small>进入相关流程</small></button>`).join("")}</div>`}
