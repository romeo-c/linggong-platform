...
window.Linggong.admin = {
  render(view) {
    ...
    if (view === "workerOps") return workerOps();
...
function companyCreateRecords(){const all=Linggong.ui.sampleRows([["OP202607300018","杭州星澜科技有限公司","91330100MA2B8X9K6A","有限责任公司","王悦（运营专员）","2026-07-30 11:26:48"],["OP202607300017","上海远屿品牌管理有限公司","91310115MA1K4B8A2C","有限责任公司","陈航（运营主管）","2026-07-30 10:08:16"],["OP202607290016","广州知行数字营销有限公司","91440101MA5C8M3D6P","有限责任公司","王悦（运营专员）","2026-07-29 16:45:32"]]);const rows=Linggong.ui.pageRows("companyCreateRecords",all);return `<p class="crumb">企业管理 / 新增企业</p>${Linggong.ui.header("新增企业","展示由运营人员代企业录入的企业新增记录。",`<button class="primary" onclick="Linggong.go('createCompanyForm')">新增企业</button>`)}<div class="table audit-table company-record-table"><table><thead><tr><th>编号</th><th>企业名称</th><th>统一社会信用代码</th><th>企业类型</th><th>提交人员</th><th>提交时间</th><th>操作</th></tr></thead><tbody>${rows.map(r=>`<tr><td>${r[0]}</td><td>${r[1]}</td><td>${r[2]}</td><td>${r[3]}</td><td>${r[4]}</td><td>${r[5]}</td><td><button class="plain" onclick="Linggong.notice('已查看企业录入记录')">查看</button></td></tr>`).join("")}</tbody></table></div>${Linggong.ui.pagination("companyCreateRecords",all.length)}`}
function companyOps(){const all=Linggong.ui.sampleRows([["CP202607001","杭州星澜科技有限公司","91330100MA2B8X9K6A","2026-07-30 11:26:48","正常"],["CP202607002","上海远屿品牌管理有限公司","91310115MA1K4B8A2C","2026-07-30 10:08:16","正常"],["CP202607003","广州知行数字营销有限公司","91440101MA5C8M3D6P","2026-07-29 16:45:32","已停用"]]);const rows=Linggong.ui.pageRows("companyOps",all);return `<p class="crumb">企业管理 / 运营管理</p>${Linggong.ui.header("企业运营管理","管理企业账号、登录状态与基础运营信息。",`<button class="secondary" onclick="Linggong.notice('企业数据已导出')">导出企业</button>`)}<div class="table audit-table company-ops-table"><table><thead><tr><th>编号</th><th>企业名称</th><th>统一社会信用代码</th><th>末次登录时间</th><th>账号状态</th><th>操作</th></tr></thead><tbody>${rows.map(r=>`<tr><td>${r[0]}</td><td>${r[1]}</td><td>${r[2]}</td><td>${r[3]}</td><td><span class="state state-${r[4]==="正常"?"success":"error"}">${r[4]}</span></td><td><button class="plain" onclick="Linggong.notice('已查看企业账号详情')">查看</button>　<button class="plain" onclick="Linggong.notice('${r[4]==="正常"?"账号已停用":"账号已启用"}')">${r[4]==="正常"?"停用":"启用"}</button></td></tr>`).join("")}</tbody></table></div>${Linggong.ui.pagination("companyOps",all.length)}`}
function workerOps(){const all=Linggong.ui.sampleRows([["WK202607300001","138****2026","林然","3301**********3821","正常","2026-07-30 11:26:48","内容运营、短视频","4.9"],["WK202607300002","139****1888","周予安","3101**********7614","正常","2026-07-30 10:08:16","活动执行、直播","4.8"],["WK202607300003","136****5632","陈星禾","4401**********2057","已停用","2026-07-29 16:45:32","平面设计、插画","4.7"],["WK202607300004","137****9014","许清","5101**********4986","正常","2026-07-29 14:18:05","翻译、会务服务","4.9"]]),rows=Linggong.ui.pageRows("workerOps",all);return `<p class="crumb">零客管理 / 运营管理</p>${Linggong.ui.header("零客运营管理","管理零客账号、技能标签和履约表现。",`<button class="secondary" onclick="Linggong.notice('零客数据已导出')">导出零客</button>`)}<div class="table audit-table worker-ops-table"><table><thead><tr><th>编号</th><th>手机号</th><th>姓名</th><th>身份证号</th><th>账号状态</th><th>注册时间</th><th>技能</th><th>履约评分</th><th>操作</th></tr></thead><tbody>${rows.map(r=>`<tr><td>${r[0]}</td><td>${r[1]}</td><td>${r[2]}</td><td>${r[3]}</td><td><span class="state state-${r[4]==="正常"?"success":"error"}">${r[4]}</span></td><td>${r[5]}</td><td><span class="skill-tags">${r[6].split("、").map(x=>`<i>${x}</i>`).join("")}</span></td><td><b>${r[7]}</b></td><td><button class="plain" onclick="Linggong.notice('已查看零客账号详情')">查看</button>　<button class="plain" onclick="Linggong.notice('${r[4]==="正常"?"账号已停用":"账号已启用"}')">${r[4]==="正常"?"停用":"启用"}</button></td></tr>`).join("")}</tbody></table></div>${Linggong.ui.pagination("workerOps",all.length)}`}
function companyReview(){const f=Linggong.state.companyReviewFilter||"全部",all=Linggong.ui.sampleRows([["XS202607300001","杭州星澜科技有限公司","待审核","2026-07-30 09:18:26","—"],["XS202607300002","上海远屿品牌管理有限公司","已审核","2026-07-30 09:02:11","2026-07-30 10:26:48"],["XS202607290018","广州知行数字营销有限公司","已审核","2026-07-29 16:45:32","2026-07-29 17:08:05"],["XS202607290017","北京启程文化传媒有限公司","待审核","2026-07-29 15:21:09","—"]]).filter(x=>f==="全部"||x[2]===f),rows=Linggong.ui.pageRows("companyReview",all);return `<p class="crumb">企业管理 / 新签审核</p><div class="heading"><div><h1>新签审核</h1><p>审核企业提交的注册、认证及签约资料。</p></div></div><div class="tabs audit-tabs">${["全部","待审核","已审核"].map(x=>`<button class="${f===x?"active":""}" onclick="Linggong.setCompanyReviewFilter('${x}')">${x}</button>`).join("")}</div><div class="table audit-table"><table><thead><tr><th>编号</th><th>企业名称</th><th>审核状态</th><th>提交时间</th><th>审核时间</th><th>操作</th></tr></thead><tbody>${rows.map(r=>`<tr><td>${r[0]}</td><td>${r[1]}</td><td><span class="state state-${r[2]==="待审核"?"warning":"success"}">${r[2]}</span></td><td>${r[3]}</td><td>${r[4]}</td><td><button class="plain" onclick="Linggong.notice('${r[2]==="待审核"?"已进入审核详情":"已查看审核记录"}')">${r[2]==="待审核"?"审核":"查看"}</button></td></tr>`).join("")}</tbody></table></div>${Linggong.ui.pagination("companyReview",all.length)}`}
