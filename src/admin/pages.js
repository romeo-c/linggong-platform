window.Linggong.adminMenu = [
  ...
  { label: "企业管理", view: "companies", children: [["新增企业", "createCompany"], ["新签审核", "review"], ["运营管理", "companyOps"]] },
...
function createCompany(){return `<p class="crumb">企业管理 / 新增企业</p><div class="heading"><div><h1>新增企业</h1><p>由运营人员代企业录入系统基础信息，保存后进入企业管理。</p></div></div><form class="form company-form" onsubmit="event.preventDefault();Linggong.notice('企业信息已保存');Linggong.go('companies')"><h2>企业基本信息</h2><div class="form-grid"><label>企业名称<input placeholder="请输入企业全称"></label><label>统一社会信用代码<input placeholder="请输入18位统一社会信用代码"></label><label>企业类型<select><option>有限责任公司</option><option>股份有限公司</option><option>个体工商户</option></select></label><label>所属行业<select><option>现代服务业</option><option>文化传媒</option><option>信息技术</option></select></label><label>法定代表人<input placeholder="请输入法定代表人姓名"></label><label>注册地址<input placeholder="请输入省、市、区"></label></div><h2>联系人与结算信息</h2><div class="form-grid"><label>联系人姓名<input placeholder="请输入联系人姓名"></label><label>联系人手机号<input placeholder="请输入手机号"></label><label>结算账户名称<input placeholder="请输入企业账户名称"></label><label>开户银行<input placeholder="请输入开户银行"></label><label class="wide">备注<textarea placeholder="可填写运营跟进说明或补充信息"></textarea></label></div><div class="foot"><button class="secondary" type="button" onclick="Linggong.go('companies')">取消</button><button class="primary">保存企业信息</button></div></form>`}
