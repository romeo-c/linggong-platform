window.Linggong.adminMenu = [
  ...
  { label: "企业管理", view: "companies", children: [["新签审核", "review"], ["运营管理", "companyOps"]] },
...
window.Linggong.admin = {
  render(view) {
    ...
    if (view === "createCompany") return createCompany();
...
function createCompany(){return `<p class="crumb">企业管理 / 运营管理 / 新增企业</p><div class="heading"><div><h1>新增企业</h1><p>由运营人员代企业录入系统基础信息，保存后返回新增记录。</p></div></div><form class="form company-form" onsubmit="event.preventDefault();Linggong.notice('企业信息已保存');Linggong.go('companyOps')"><h2>企业基本信息</h2><div class="form-grid"><label>企业名称<input placeholder="请输入企业全称"></label><label>统一社会信用代码<input placeholder="请输入18位统一社会信用代码"></label><label>企业类型<select><option>有限责任公司</option><option>股份有限公司</option><option>个体工商户</option></select></label><label>所属行业<select><option>现代服务业</option><option>文化传媒</option><option>信息技术</option></select></label><label>法定代表人<input placeholder="请输入法定代表人姓名"></label><label>注册地址<input placeholder="请输入省、市、区"></label></div><h2>联系人与结算信息</h2><div class="form-grid"><label>联系人姓名<input placeholder="请输入联系人姓名"></label><label>联系人手机号<input placeholder="请输入手机号"></label><label>结算账户名称<input placeholder="请输入企业账户名称"></label><label>开户银行<input placeholder="请输入开户银行"></label><label class="wide">备注<textarea placeholder="可填写运营跟进说明或补充信息"></textarea></label></div><div class="foot"><button class="secondary" type="button" onclick="Linggong.go('companyOps')">取消</button><button class="primary">保存企业信息</button></div></form>`}
function companyOps(){const rows=[["OP202607300018","杭州星澜科技有限公司","91330100MA2B8X9K6A","有限责任公司","王悦（运营专员）","2026-07-30 11:26:48"],["OP202607300017","上海远屿品牌管理有限公司","91310115MA1K4B8A2C","有限责任公司","陈航（运营主管）","2026-07-30 10:08:16"],["OP202607290016","广州知行数字营销有限公司","91440101MA5C8M3D6P","有限责任公司","王悦（运营专员）","2026-07-29 16:45:32"]];return `<p class="crumb">企业管理 / 运营管理</p>${Linggong.ui.header("企业运营管理","展示由运营人员代企业录入的企业新增记录。",`<button class="primary" onclick="Linggong.go('createCompany')">新增企业</button>`)}<div class="table audit-table company-record-table"><table><thead><tr><th>编号</th><th>企业名称</th><th>统一社会信用代码</th><th>企业类型</th><th>提交人员</th><th>提交时间</th><th>操作</th></tr></thead><tbody>${rows.map(r=>`<tr><td>${r[0]}</td><td>${r[1]}</td><td>${r[2]}</td><td>${r[3]}</td><td>${r[4]}</td><td>${r[5]}</td><td><button class="plain" onclick="Linggong.notice('已查看企业录入记录')">查看</button></td></tr>`).join("")}</tbody></table></div>`}
